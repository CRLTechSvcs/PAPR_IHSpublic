 function populatedialog1(response, ioArgs){
		...
		function getDataErrorDetail(i){
		  ...
		  dojo.xhrGet({
        			...
        			preventCache: true,
        			error: function (e) {
            			alert("ihs_exception.js, getDataErrorDetail Error: " + e.message);
        			},
		...
		function reprocessDataErrorRecord(){
		    ...
		    dojo.rawXhrPost ({
					...
					error: function(error) {
						alert("ihs_exception.js, reprocessDataErrorRecord Error:"+ error);
					}
				});
		}
		...
		function ignore(){
			...
			dojo.rawXhrPost ({
					...
					error: function(error) {
						alert("ihs_exception.js, function ignore() Error:"+ error);
					}
				});
		}
		...
		function onhold(){
		    ...
		    dojo.rawXhrPost ({
					...
					error: function(error) {
						alert("ihs_exception.js, onhold Error:"+ error);
					}
				});
	    ...
	    function getException (){
	    	 ...
	    	 dojo.xhrGet({
        			...
        			preventCache: true,
        			error: function (e) {
            			alert("ihs_exception.js, getException Error: " + e.message);
        			},
